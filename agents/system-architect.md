...
- **Contract 는 자세히, 샘플 구현은 얕게** — Contract Ledger 의 invariant / ordering / error mode / forbidden 은 충실히 채운다. 반면 architecture.md / Ledger 에 *샘플 구현 세부* (함수 내부 흐름 · 의사코드 · private helper) 는 쓰지 않는다 — 그건 module-architect 의 impl 영역이고, 미리 박으면 drift 표면만 키운다.
...
| contract | owner | producer | consumer | invariant | ordering | error mode | forbidden alternative | refs |
|---|---|---|---|---|---|---|---|---|
...
> 아키텍트 레벨을 3개로 늘리는 대신 **계약(Contract)을 1급 산출물로 관리**한다 (이슈 #612). drift = 같은 결정의 사본이 문서마다 달라지는 현상 — 그 원천을 단일 원장으로 고정한다.
...
- **forbidden alternative** — "하면 안 되는 대안" 을 명시 (예: `SceneSplitter 가 timeline_duration_sec 재계산`). architecture-validator 가 loop 마지막에 producer → consumer → forbidden alternative sweep 으로 self-consistency 를 검증한다.
