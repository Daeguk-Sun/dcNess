...
| Agent effectiveness | 측정 계약 검증 PASS — 실제 agent effectiveness는 측정 불가 | baseline/current trial 4, task 2 / synthetic fixture 1 | [`docs/internal/outcome-baseline.md`](docs/internal/outcome-baseline.md#2026-07-12-agent-effectiveness-deterministic-screening)의 고정 record 명령 | record trace는 실제 agent run에서 유래하지 않은 저작된 synthetic 값이다(run ID·raw trace 없음). effectiveness 개선 관측이 아니다 |
