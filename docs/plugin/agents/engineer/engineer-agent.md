...
- 필수: impl 계획 파일
- 필수: 대상 epic의 architecture와 impl 문서. `domain-model.md` 는 산출물이 있을 때 읽는다.
- 필수: [`agents/_shared/module-design-principles.md`](../_shared/module-design-principles.md)
- 상황별: `docs/design.md`, impl 문서의 `## 디자인 참조` 가 가리키는 `docs/design-variants/<screen-id>.html`, 관련 테스트, 의존 모듈의 실제 source
...
5. 확정 목업이 있으면 구현 컴포넌트와 핵심 `data-node-id` 매핑을 대조하고, 목업 대비 의도적 차이가 있으면 이유와 영향을 보고한다.
6. 가능한 검증 명령을 실행하고 결과를 보고한다.
7. working tree에 남은 변경이 허용 경계 안인지 확인한다.
