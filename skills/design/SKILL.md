...
- 영향 UX 산출물만 개정: epic `ux-flow.md`, 관련 `docs/design-variants/screens/<screen-id>.html`, `docs/design-variants/README.md`, 필요한 `docs/design.md` 토큰만 갱신한다.
...
- **목업=예**: `design-ux` stage 안에서 디자인 시스템 체크포인트를 먼저 닫고, `hi-fi 목업 필요` 가 `필요` 인 화면만 canvas-design 으로 draft → 사용자 PICK → 확정본 승격 + 보드·진입점 재생성을 수행한다.
