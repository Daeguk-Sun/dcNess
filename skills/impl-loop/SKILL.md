...
- **loop**: `impl-task-loop` (UI 감지 시 `impl-ui-design-loop` — engine 무관 `canvas-design` + 사용자 PICK 선두 추가, 아래 `## UI 작업 시 canvas-design 선두`)
- **entry_point**: `impl`
- **task_list** (Step 1): (풀 4-agent, default=single) test-engineer → engineer:IMPL → code-validator → pr-reviewer · (build-worker, default=chain) build-worker → pr-reviewer · (advanced fallback: deep task 보강 필요 시 module-architect 선두 추가) · (impl-ui-design-loop) canvas-design → 사용자 PICK 선두
- **advance**: `PASS` → `IMPL_DONE` → `PASS` → `PASS` (풀 4-agent) · `PASS` → `PASS` (build-worker) · `PASS`(canvas-design) → 사용자 PICK → 각 엔진 구현 step (impl-ui-design-loop)
- **expected_steps**: 4 (풀) / 5 (advanced fallback) / 2 (build-worker) · UI 풀 4-agent = 6 / UI build-worker = 4 / UI advanced fallback = 7 · story 마감 task +1 / epic 마감 task +2 (`product-acceptance`, 아래 `## 마감 acceptance`)
...
UI 작업 감지 시 engine 무관하게 구현 시퀀스 선두에 내부 [`canvas-design`](../canvas-design/SKILL.md) 절차를 둔다. build-worker 와 풀 4-agent 모두 같은 확정 목업을 읽고 구현한다.

- **canvas-design** (`PASS,ESCALATE`) — 기준 있음 / 신규 시각 구조 + 기준 없음 / 시각 구조 불변을 판정하고, 필요한 경우 designer draft 생성 → 사용자 PICK → 확정본 승격 → `docs/design-variants/canvas.html` frame 등록을 수행한다. 반환값은 `docs/design-variants/<screen-id>.html` 확정 목업 경로와 핵심 node-id 매핑이다.
- **사용자 PICK** (helper begin/end-step 비대상) — 메인이 draft 경로 + node-id 안내 + OK/NG 를 받는다. NG 시 canvas-design 내부에서 designer draft 를 재생성한다. round 한도 X.

이후 구현 step 은 선택된 엔진을 따른다. build-worker 엔진이면 build-worker → pr-reviewer, 풀 4-agent 엔진이면 test-engineer → engineer:IMPL → code-validator → pr-reviewer 다. deep task 보강이 필요하면 canvas-design 앞에 module-architect 1 step. canvas-design `ESCALATE` → 사용자 위임.
...
- UI 감지 → 선두에 canvas-design + 사용자 PICK (위 `## UI 작업 시 canvas-design 선두`).
