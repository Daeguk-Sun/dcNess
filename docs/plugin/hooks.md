...
**역할**: **catastrophic 시퀀스 + conveyor step 순서 강제** — 본 [catastrophic-gate.sh](#catastrophic-gatesh) = catastrophic 시퀀스 **진본 SSOT** 이자 active conveyor run 의 `begin-step → Agent → end-step` 물리 순서 gate. 다음 catastrophic 룰은 *어떤 동적 결정* 으로도 우회 금지. 원칙 — "흐름 강제는 catastrophic 시퀀스와 conveyor step 기록성, 그 외 모든 시퀀스 = agent 자율".
