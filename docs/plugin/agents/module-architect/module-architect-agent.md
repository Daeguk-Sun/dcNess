...
10. **impl-task (Story/공통 task 분할 산출물) 한정으로** frontmatter 의 risk 메타(`risk` / `engine` / `risk_reason`)를 **task 를 자르는 시점에 함께 판정해 적는다** (compact plan 은 `/impl` Standard 구현 경로 산출물 — impl-loop dry preview 비대상이라 risk 메타 비적용). 엔진 판정 기준은 [`workflow-router.md`](../../workflow-router.md) high-risk trigger 표가 아니다. workflow-router high-risk trigger 표는 설계 선행 판정 전용이고, full `/design` 산출 impl-task 는 이미 architecture 산출물과 architecture-validator 검증으로 설계 시점 위험을 소화한 상태다. impl-task 엔진 판정의 SSOT 는 이 문서의 **구현 시점 위험** 기준이다.
