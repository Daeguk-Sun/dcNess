...
| 구현 경로 | `lane` | `/impl` 안에서 사용자에게 echo 하는 내부 처리 이름. 현재 값은 direct / design-doc 다. | concrete signal 또는 충분한 자연어 구현 의도가 있으면 direct 경로다. | [`workflow-router.md`](workflow-router.md), [`skills/impl/impl-routing.md`](../../skills/impl/impl-routing.md) |
...
| 진행 순서 검사 | `strict conveyor`, `strict-conveyor`, `conveyor` | active run 안에서 호출 적합성과 lifecycle identity를 검사하는 hook/helper 영역. mode 없는 foreground Claude Agent는 actual spawn/완료 hook, modeful·headless는 explicit helper 경계를 쓴다. | 진행 순서 검사는 설계 선행조건이나 current mode/agent identity가 맞지 않으면 차단한다. | [`hooks.md`](hooks.md#catastrophic-gatesh), [`loop-procedure.md`](loop-procedure.md#표준-1-step-시퀀스-per-agent-의무) |
...
| task REQ | Story AC 를 task가 실행·관찰할 검증 언어로 번역한다. 제품 REQ 는 `(from AC-NNN)`, 소수 기술 REQ 는 `(technical: 이유)`를 쓰며, 검증방법은 `(TEST)`·`(AGENT READ)`·실앱 자동화 `(JOURNEY)` 중 하나다. | build-worker가 `(TEST)`·`(AGENT READ)`를 실행하고 `(JOURNEY)` flow/매니페스트를 작성한 뒤 final tip 수렴 호출로 실행 가능 상태까지 고친다. product-acceptance는 별도 sealed 실행으로 `(JOURNEY)`를 판정한다. | [`docs/plugin/agents/module-architect/templates/impl-task.md`](agents/module-architect/templates/impl-task.md) |
