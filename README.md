...
|---|---|---|---|---|
| 하네스·기계적 guard | unittest 1,925/1,925 PASS, 결정적 guard 39/39 PASS | test 1,925, guard case 39 / dcNess checkout 1 | `node scripts/check_public_evidence.mjs` | 보안 증명이나 제품 성공률이 아니다 |
| Agent effectiveness | 실측 개선 관측 — 오경로 `1→0`, 영향 과다 포함 `2→0`, 탐색 tool `14→13` (품질 비열화 없음) | baseline/current trial 4, task 2 / 실측 fixture 1 | [`docs/internal/outcome-baseline.md`](docs/internal/outcome-baseline.md#2026-07-13-agent-effectiveness-실측-paired-screening)의 record 재현 명령 | model `claude-sonnet-4-6` 단일 frozen fixture 1회 paired 실측(1+1). 1차 시도는 계약 미충족으로 거부됐고 계약 개선 후 2차를 채택했으며 세션 ID·raw trace·SHA-256·생성 명령이 record provenance에 있다. 공개 우위 주장이 아니다 |
