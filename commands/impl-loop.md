...
## 절차 — in-session 순차 진행 (Hybrid A, #446)
...
> **Hybrid A 분기** — 정식 위치 impl 파일 (`docs/milestones/v\d+/epics/epic-\d+-*/impl/\d+-*.md`) 존재 시 build-worker 2-step. 부재 시 fallback (4-agent + module-architect 선두). 자세히 = [`orchestration.md §4.3 진입 모드`](../docs/plugin/orchestration.md). `/impl` 단발 호출은 본 Hybrid A 분기 미적용 — 4-agent 모델 유지 (rigor 우선 티어링).
...
2. **task 한 개씩** — 아래 a~e 를 task N 이 완전히 끝난 (PR 머지 + 이슈 close) 뒤에만 task N+1 진입. 한 번에 한 task.
   ...
   4. **pr-reviewer step + 머지** — `begin-step pr-reviewer` → `Agent(pr-reviewer, prompt=<PR 번호 + impl 경로 + 구현 파일 목록>)` → `PASS` 시 `gh pr merge --auto --merge` + CI watch → 머지 완료. `FAIL` 시 engineer POLISH 단발 진입 (cycle ≤ 2). 이후 `end-step pr-reviewer`.
   5. **end-run + review echo (digest override)** — `end-run` 발사. 메인 컨텍스트에 박는 echo 는 본 skill §"review echo override (#446)" 의 5줄 digest. verbatim review.md 는 `<run_dir>/review.md` 디스크 그대로.
3. **enum 별 분기**:
   - `clean` → 다음 task 진입
...
정식 위치 impl 파일 부재 시 build-worker 미사용. 대신 4-agent (test-engineer / engineer:IMPL / code-validator / pr-reviewer) + module-architect 선두 = 5-step 풀스펙. 이 경우 절차 §2.ii 는 [`commands/impl.md`](impl.md) 본문 의무 그대로 (4-agent 시퀀스). 워크트리는 outer 1회.

### 세션 청킹 권장 (#446)

15-20 task/세션 권장. 그 이상이면 사용자가 명시적으로 `/compact` 또는 새 세션 진입 — 메인 컨텍스트 cache_read 비용 곡선 (~5-min TTL 후 절벽) 회피. resume = §"compaction 중 진행" 안전망 그대로. 세션 청킹은 의도된 멈춤 — §진행 뷰 의 `✓` 헤더 보고 사용자가 자연 멈춤 결정.
...
fallback 모드 (impl 부재) 는 sub-step 5 (`module-architect` / `test-engineer` / `engineer` / `code-validator` / `pr-reviewer`).
...
- sub-step 수 = **2 (Hybrid A, /impl-loop default)** / 5 (fallback: impl 부재 시 module-architect 선두)

**진입 시** — TaskCreate 를 이 순서로: `task1 헤더` → `task1 sub × 2 (Hybrid A) 또는 × 5 (fallback)` → `task2 헤더` → … → `taskN 헤더`. task1 헤더만 in_progress.

**task i 진행 중** — 그 task 의 `impl` run 은 TaskCreate skip (sub-step task 이미 존재 — loop-procedure §2). conveyor 진행에 맞춰 sub-step 을 TaskUpdate (pending → in_progress → completed). Hybrid A 의 경우 build-worker 안의 3 phase (build-test / build-impl / build-validate) 는 outer sub-step `build-worker` 하나로 묶임 — `activeForm` 에 phase 표시.
...
fallback 모드 (impl 부재, 4-agent + module-architect) 일 때 2번째 줄은 `module-architect: <PASS|SPEC_GAP> · test-engineer: <N tests RED→GREEN> · engineer: <M files +X -Y> · code-validator: <PASS|FAIL> · pr-reviewer: <LGTM|FAIL>`.
...
- worker phase prose 자체 Write 규약: [`loop-procedure.md §3.2.1`](../docs/plugin/loop-procedure.md)
