...
산출 형식은 [`spec-stories-reference.md`](spec-stories-reference.md)를 따른다. 각 Story 에 프로젝트 전역 불변 `AC-NNN` 목록을 만들고, 사람 판정 항목은 Story AC 가 아닌 별도 안내로 분리한다. Story 분할과 순서도 같은 reference 의 사용자 검증 가능한 동작 증분 / 얇은 골격 우선 기준을 따른다.
