...
#   2. harness/ | tests/ | agents/ | docs/plugin/agents/ (지침·templates·references·_shared 이동처, 계약 테스트 소비) | skills/ | commands/ | templates/github-workflows/ | audited scripts (aggregate_*_map, scripts/lib/ 파생 helper 포함) | doc-path/doc-sync action/script | .github/workflows/python-tests.yml 매칭 시만 실행
...
if ! printf '%s\n' "$CHANGED" | grep -qE '^(harness/|tests/|agents/|skills/|commands/|templates/github-workflows/|evals/|\.github/actions/(doc-path-integrity|doc-sync)/|scripts/design/|scripts/check_git_naming\.mjs$|scripts/check_doc_path_integrity\.mjs$|scripts/check_design_artifact_structure\.mjs$|scripts/dcness-story-runner$|scripts/aggregate_(index|architecture)_map\.mjs$|scripts/lib/|docs/plugin/agents/|docs/plugin/git-spec\.md$|docs/plugin/loop-procedure\.md$|\.github/workflows/python-tests\.yml$)'; then
  # 미매칭 — skip
  exit 0
fi
