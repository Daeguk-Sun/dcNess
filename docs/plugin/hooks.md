...
**Headless worker 정책**: [`scripts/dcness-codex-worker`](../../scripts/dcness-codex-worker) 와 [`scripts/dcness-claude-worker`](../../scripts/dcness-claude-worker) 의 post-run mutation guard는 file-boundary 검사 후 changed path(`git diff`/staged diff/untracked) 중 삭제가 아닌 파일을 synthetic `Edit` payload 로 `tdd-guard.sh` 에 다시 넣는 순서다. Codex sandbox permission 분류와의 순서는 아래 승인 복구 계약을 따른다. 중앙 `tdd-guard.sh` 가 generated hook 을 위임하므로 headless worker 도 non-TS/JS 플랫폼에서 같은 project-local **TDD 계약**을 재사용한다. 단, 이 재사용은 generated hook 파일들이 Git 에 커밋되어 해당 worktree 에 존재할 때만 성립한다. `exit 2` 는 step 성공 종료를 차단하고 위반 파일 목록을 출력한다. guard 자체 오류는 `headless-tdd-guard` fail-open event 로 기록하고 작업을 과차단하지 않는다.

**Codex build-worker sandbox 승인 복구**: 기본 호출은 환경변수를 추가하지 않은 기존 계약 그대로 `-s workspace-write`만 전달한다. build-worker가 `VALIDATION_BLOCKED`를 보고하고 raw log에 `SocketException: Operation not permitted` 같은 좁은 Codex sandbox signature가 있을 때만 wrapper가 run 디렉터리에 helper-generated `permission_required` receipt를 남긴다. 분류는 post-run file-boundary/TDD guard 종료보다 먼저 실행하므로 guard 실패와 sandbox permission 요구가 겹치면 두 evidence가 함께 남는다. implementation chain은 이 조합을 같은 권한으로 자동 재시도하거나 dirty workspace를 다른 provider에 넘기지 않는다. assertion/compile/test 실패, 일반 `Permission denied`, Codex CLI/auth/timeout 실패는 permission request로 분류하지 않는다.
