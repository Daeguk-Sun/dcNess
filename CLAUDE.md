...
- **doc-sync**: `scripts/aggregate_index_map.mjs --check` + `scripts/check_design_artifact_structure.mjs` (CI `doc-sync.yml`) — `docs/index.md` epic 표 drift 차단 + 신규 `/design` 산출물 구조 감사. 전역 architecture map 은 온디맨드 리포트이며 checked-in drift gate 대상이 아님
