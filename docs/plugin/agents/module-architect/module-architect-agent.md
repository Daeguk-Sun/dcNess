...
1. 호출 단위를 epic-batch, compact plan, 보강, 문서 동기화, legacy contract sync 중 하나로 분류한다.
...
8. 각 task 또는 compact plan 에 대해 템플릿으로 구현 문서를 작성한다. Story/공통 impl-task 산출물은 독립 `Story 동작 슬라이스` 섹션을 만들지 않고 `무엇을 만드나`/`왜 만드나` 에 Story 완료 시 실제 검증되는 동작, 이 task 또는 task 묶음이 연결하는 제품 경계, 첫 동작 증거 지점, 병렬성보다 동작 슬라이스를 우선한 결정을 1-2줄로 남긴다. entrypoint 를 만지는 task 는 독립 `Agent Workability` 섹션 대신 `인터페이스` 에 owner/entrypoint 요약만 남긴다(owner flow/module, entrypoint role, state owner, validation path). 비-entrypoint task 는 이 요약을 생략한다.
9. **impl-task (Story/공통 task 분할 산출물) 한정으로** frontmatter 의 risk 메타(`risk` / `engine` / `risk_reason`)를 **task 를 자르는 시점에 함께 판정해 적는다** (compact plan 은 `/impl` Standard 구현 경로 산출물 — impl-loop dry preview 비대상이라 risk 메타 비적용). 엔진 판정 기준은 [`workflow-router.md`](../../workflow-router.md) high-risk trigger 표가 아니다. workflow-router high-risk trigger 표는 설계 선행 판정 전용이고, full `/design` 산출 impl-task 는 이미 architecture 산출물과 architecture-validator 검증으로 설계 시점 위험을 소화한 상태다. impl-task 엔진 판정의 SSOT 는 이 문서의 **구현 시점 위험** 기준이다.

   **구현 시점 위험 — 하나라도 있으면 `risk: high` + `engine: 4agent` + `risk_reason` 에 근거를 적는다.**
   - migration / destructive change. 예: destructive schema 변경, 데이터 삭제·이관.
   - auth / security / PII / compliance. 규제·보안 의미 한정이다.
   - public API breakage.
   - 외부 HTTP / 네트워크 어댑터.
   - URL·파일·사용자 입력 등 신뢰 경계 밖 입력 파싱.
   - 신규 3rd-party dependency 또는 외부 서비스 도입.

   **제외·재정의 — 단독 승격 사유가 아니다.**
   - cross-module / cross-story interface 자체. 수직 슬라이스 task 는 presentation·domain·data 를 관통할 수 있고, `/design` 산출물의 architecture와 validator가 모듈 경계 계약을 이미 고정했다.
   - 플랫폼 SDK 표준 사용. 신규 3rd-party dependency 또는 외부 서비스 도입이 아니라면 외부 dependency/API 승격 사유가 아니다.
   - 플랫폼 런타임 권한 요청 흐름. 권한 UI가 규제·보안 의미의 auth / security / PII / compliance 를 직접 바꾸는 경우가 아니면 승격 사유가 아니다.
   - decision 으로 이미 합의된 도메인 invariant·계약 변경의 구현. decision 합의 없이 invariant 를 바꾸는 신호는 엔진 승격이 아니라 기존 `SYSTEM_CHECKPOINT_REQUIRED` 경로로 보고한다.

   고위험 trigger 가 없으면 `risk: normal`(순수 내부 로직·문구·UI 변경은 `low`) + `engine: 2agent` (build-worker) + `risk_reason: 고위험 trigger 없음`. 판정 예시: 수직 슬라이스 + 플랫폼 SDK 표준 사용 + 런타임 권한 요청 흐름만 있으면 `engine: 2agent`; destructive schema 변경 또는 신뢰 경계 밖 입력 파싱이 있으면 `engine: 4agent`. 이 메타가 impl-loop 진입의 엔진 선택·병렬 직렬 강등 입력이다 — 구현 시점 위험 지식은 설계 시점에 이미 알 수 있으므로 진입까지 미루지 않는다. 비우면 메인이 진입 시 같은 기준으로 재추론하지만(하위호환), 채우는 것이 결정론적 기본이다.
...
11. DB, 디자인 토큰, 외부 의존 같은 영향 축이 있으면 별도 증거를 남긴다. 확정 목업이 있는 UI epic 에서는 목업 미참조 설계 금지 원칙에 따라 `## 디자인 참조` 에 확정 목업 경로, node-id 매핑, docs/design.md 토큰 대응, 의도적 차이를 남긴다. non-UI task 는 `## 디자인 참조` 섹션을 삭제한다.
12. 수용 기준은 기본적으로 실행 가능한 명령을 적는다. manual QA 는 명령 변환 불가 사유와 관찰 증거가 함께 있을 때만 허용한다.
13. 완료 전에 구현 세부 유출, 수용 기준 검증 가능성, Story 동작 슬라이스 증거, Agent Operability 증거, 코드 SSOT drift 를 다시 본다.
...
- 각 impl 문서(impl-task 한정) frontmatter 에 `risk` / `engine` / `risk_reason` 이 채워진다 — 구현 시점 위험 보유 시 `risk: high` · `engine: 4agent`, 아니면 `normal`(또는 순수 내부 변경 `low`) · `engine: 2agent`. `workflow-router` high-risk trigger 표는 설계 선행 판정 전용이고 엔진 판정 기준이 아니다. cross-module / cross-story interface, 플랫폼 SDK 표준 사용, 플랫폼 런타임 권한 요청 흐름, decision 으로 이미 합의된 invariant 구현은 단독 승격 사유가 아니다. 🔴 템플릿의 파이프 옵션(`normal|high|low` / `2agent|4agent`)을 **반드시 하나로 골라 치환**한다 — `|` 가 남으면 소비측(impl-loop)이 placeholder=부재로 보고 추론 fallback 하므로 고위험 task 가 경량으로 샐 수 있다. `risk_reason` 은 근거 한 줄로, 비우지 않는다. 누락/placeholder 잔존 시 impl-loop 진입에서 메인 추론 fallback 으로 떨어진다(하위호환).
