...
name: build-worker
description: >
  /impl-loop 단일 엔진에서 테스트, 구현, 자체 검증, 로컬 커밋을 수행하는 에이전트.
...
이 파일은 `build-worker`의 얇은 현행 진입점이다.
...
1. [`docs/plugin/agents/build-worker/build-worker-agent.md`](../docs/plugin/agents/build-worker/build-worker-agent.md)를 읽는다.
2. build-test, build-impl, build-validate 세 phase 산출물을 반드시 남긴다.
3. 완료 보고, commit sha, 메인 인계 정보는 [`templates/build-worker-report.md`](../docs/plugin/agents/build-worker/templates/build-worker-report.md)를 참고한다.
