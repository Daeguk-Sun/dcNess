...
4. `/impl-loop`는 `dcness-implementation-chain build-worker --provider <provider> --provider-provenance <routing|explicit> --chain-state .dcness-work/story-run.json --prompt-file <file>`을 실행한다. prompt 에는 target GitHub issue AC snapshot 을 진본 포인터로 포함한다. 성공 경로는 마지막 응답 저장과 `end-step build-worker` 까지 수행한다. `--chain-state`가 없는 single `/impl`은 provider failure cache를 사용하지 않는다.
