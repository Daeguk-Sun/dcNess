...
PR 본문 초안 ([`docs/plugin/git-spec.md`](../docs/plugin/git-spec.md) 의 PR 본문 핵심 섹션만 — 메인이 종합 갱신):
## 관련 이슈 번호
<Part of #N 또는 Closes #N — impl 계획 frontmatter task_index 기준 (PR 트레일러 기본 룰 / 적용 절차)>
task-index: <i>/<total>
...
commit message 초안 (`git-spec.md` 의 커밋 제목 + 커밋 메시지 본문):
<제목: 커밋 제목 패턴 — 예: "[epicN][storyM] <설명>" 또는 "[issue-N] <설명>">
...
결론: <PASS | SPEC_GAP_FOUND (+ 분량 메타 small/medium/large) | TESTS_FAIL | IMPLEMENTATION_ESCALATE> (+ 사유) — 다음 호출은 메인이 handoff 라우팅으로 결정
