...
- **guard 판정·runtime evidence 경량화** (Closes [#1146](https://github.com/Daeguk-Sun/dcNess/issues/1146)) — hook payload를 `HookRequest`에서 한 번 정규화하고 policy 결과를 `GuardDecision`으로 통일했다. runtime에는 복구에 필요한 state와 최소 guard/eval receipt만 남기며 per-tool 입력 trace, histogram, dashboard, 자동 lesson/insight, agent effectiveness 분석은 release artifact에서 제거했다. public `/run-review`·`/efficiency`는 각각 단일 run 복기와 read-only token/cost 요약을 유지한다. **동작 변화·migration**: 옛 `dcness-helper guard-telemetry`·`insight`와 `dcness-efficiency dashboard|patterns|patterns-dashboard|full`은 silent unknown command 대신 exit 2와 대체 경로를 안내한다. raw receipt의 기존 epoch·필드 의미는 유지하고, repository 분석은 source checkout의 `scripts/loop_diagnose.py`, token/cost는 `analyze`/`summary`, 완료 run은 `/run-review`를 사용한다. **검증**: unittest 1,121/1,121, guard efficacy 39/39, release artifact Python 18,401 LOC.
