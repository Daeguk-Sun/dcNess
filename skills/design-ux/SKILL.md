...
- 조건부: `docs/design.md`
- 조건부: `docs/design-variants/screens/<screen-id>.html`
...
5. **ux-architect 호출** — `UX_FLOW` 모드로 epic `ux-flow.md` 를 작성한다. revision mode 에서는 기존 `ux-flow.md`, `docs/design.md`, 확정 목업/보드 포인터와 "화면 통합/분할/삭제 등 UX 층 개정 의도"를 입력으로 넣고 영향 UX 산출물만 개정하게 한다. 화면 인벤토리는 `hi-fi 목업 필요` 열을 유지하고, 목업=예 경로에서는 `docs/design.md` 토큰을 참고 디자인 시스템 신호에 맞춰 정리한다.
6. **목업 필요 화면 처리** — 목업=예 경로에서만 `hi-fi 목업 필요` 가 `필요` 인 화면을 내부 [`canvas-design`](../canvas-design/SKILL.md) 으로 넘긴다. seed 보장, designer draft, 사용자 PICK, 확정본 승격, 보드·진입점 재생성은 canvas-design 계약을 따른다. 확정본 승격 후 메인은 epic `ux-flow.md` 화면 인벤토리의 `확정 목업 경로` 를 `docs/design-variants/screens/<screen-id>.html` 로 채우고, 화면별 `확정 목업` 항목에 보드 진입점과 핵심 node-id 매핑을 기록한다. system stage 는 사용자 PICK 확정 이후에만 진입한다.
