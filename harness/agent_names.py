...
(`dcness:build-worker`)를 제거한다. boundary (`agent_boundary`) / order gate /
review가 동일 canonical 이름으로 매칭하도록 본 모듈이 *단일* 정규화를 제공한다.
...
def normalize_agent_type(agent_type: Optional[str]) -> Optional[str]:
    ...
    `dcness:` namespace prefix를 제거한다. boundary / order gate 매칭 전
