...
| 구현 경로 | `lane` | `/impl` 안에서 설계도 유무에 따라 갈리는 내부 경로. 현재 값은 direct / design-doc 다. | 설계 문서가 없고 경계가 명확하면 direct 구현 경로다. | [`workflow-router.md`](workflow-router.md), [`skills/impl/impl-routing.md`](../../skills/impl/impl-routing.md) |
