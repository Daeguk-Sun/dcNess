...
class PrFinalizePeerLockWiringTests(unittest.TestCase):
    ...
    def test_pr_finalize_ci_failure_marks_peer_claim_failed(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            gh.write_text(
                "#!/bin/sh\n"
                "if [ \"$*\" = \"repo view --json defaultBranchRef -q .defaultBranchRef.name\" ]; then\n"
                "  echo main\n"
                "  exit 0\n"
                "fi\n"
                "if [ \"$*\" = \"pr view 101 --json baseRefName -q .baseRefName\" ]; then\n"
                "  echo main\n"
                "  exit 0\n"
                "fi\n"
                "if [ \"$*\" = \"pr view 101 --json headRefName -q .headRefName\" ]; then\n"
                "  echo feature/a\n"
                "  exit 0\n"
                "fi\n"
