...
6. 기존 코드의 계약 표면 코드 SSOT 대조를 수행한다. 포트, 도메인 타입, 공개 entrypoint, adapter 계약이 있으면 실측하고, impl task 가 기존 계약을 유지하는지 변경하는지 module responsibility / decision 문서와 연결한다. 이 과정에서 기존 모듈 경계, 도메인 invariant, storage policy, public API boundary, 기존 전역 decision 자체를 바꿔야 한다는 신호가 나오면 task를 계속 쓰지 말고 `SYSTEM_CHECKPOINT_REQUIRED` 로 보고한다. 단순 신규 module row 추가, 신규 epic-scope decision 기록, decision link 추가, impl task 의 Agent Workability/scope/acceptance 보강은 checkpoint 사유가 아니다.
