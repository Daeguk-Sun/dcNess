...
2. 도메인 모델과 architecture의 계약 원장을 먼저 읽는다. epic `architecture.md` 의 `## Contract Ledger` 가 cross-task 계약 전문의 단일 진본이다.
...
9. public contract를 만들거나 바꾸면 Contract Ledger를 갱신하고 impl/compact plan 은 Ledger 행 키와 갱신 사실만 남긴다. impl/compact plan 은 Ledger 행 키 포인터 문서이며, invariant/ordering/error mode/config/forbidden alternative 전문을 복제하지 않는다. task 내부 한정 private interface 는 사본 문제가 없으므로 `## 인터페이스` 에 남긴다.
