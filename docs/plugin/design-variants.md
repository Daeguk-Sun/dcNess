...
노드는 선언 경로에서 마지막으로 등장한 순서로 놓는다. 같은 화면을 여러 여정이 참조해도 `screens/` 파일은 하나다. 여러 ux-flow가 같은 화면을 참조하면 인벤토리 ID·이름·설명이 같아야 하며, 충돌 시 생성기는 flow 경로와 값을 함께 알린다. 위치·간격·곡률은 보드 파일에 기록하지 않고 엔진이 경로와 렌더 결과에서 파생한다.
