...
- **designer** (`PASS,ESCALATE`) — static HTML 시안 생성. 산출물은 single-file, no-build HTML 이며 `design-variants/<screen>-v<N>.html`, `data-node-id`, `:root` CSS custom property 토큰을 포함한다.
- **사용자 PICK** (helper begin/end-step 비대상) — 메인이 HTML 시안 경로 (`design-variants/<screen>-v<N>.html`) + node-id 안내 + OK/NG. NG 시 designer 재호출 (sub_cycle `designer-ROUND-<n>`, round 한도 X).
