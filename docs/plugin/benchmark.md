...
| Agent effectiveness | 측정 계약 검증 PASS — 실제 agent effectiveness는 측정 불가 | baseline/current trial 4, task 2 | synthetic fixture 1 | `printf '{"version":1,"projects":[]}' > /tmp/dcness-empty-projects.json && python3.11 harness/outcome_scorecard.py --projects-file /tmp/dcness-empty-projects.json --agent-effectiveness-record evals/agent-effectiveness/cartography-sanity-replay.json --measured-at 2026-07-12T10:00:00Z --json` | record trace는 실제 agent run에서 유래하지 않은 저작된 synthetic 값이다(run ID·raw trace 없음). deterministic 저장 replay이며 effectiveness 개선 관측이 아니다 |
