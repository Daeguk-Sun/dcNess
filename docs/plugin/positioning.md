...
| `/impl` | 구현, 수정, 버그픽스, 작은 리팩터링을 실제 PR 로 끝낼 때 | issue-intake/direct/design-doc echo + review provider 를 내부 판정 |
