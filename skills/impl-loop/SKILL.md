...
- **고위험 task 자동 승격** (frontmatter `risk` 부재 시 추론 fallback): chain 기본이 build-worker 여도 task Scope/파일 경로가 외부 HTTP·네트워크 어댑터, URL/파일/사용자 입력 파싱, auth/security/PII, 도메인 invariant 변경을 포함하면 그 task만 풀 4-agent로 올린다 — frontmatter `risk: high` 가 명시돼 있으면 추론 없이 그 값으로 승격한다. self-grading drift 비용이 과승격 비용보다 크다. 단순 UI/문구/순수 내부 도메인 task는 경량 유지한다.
