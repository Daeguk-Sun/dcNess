...
6. `begin-step impl-validator CODEBASE_SANITY` → 같은 read-only impl-validator에 final merge candidate, task별 build-worker 보고, 수집한 기계적 증거, scope, Cartography 상태를 전달한다. finding이면 build-worker rework로 코드를 고친 뒤 새 revision에서 Sanity를 재감사한다. PASS이면 prose의 최소 의미를 `.dcness-work/codebase-sanity/<tree-identity>.md` local receipt로 보존한다. local-only/ignored 정책이면 code PR에 포함하지 않는다.
