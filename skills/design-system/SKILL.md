...
4. **Codebase Sanity receipt freshness preflight** — `.dcness-work/codebase-sanity/`의 직전 Codebase Sanity receipt가 기록한 code revision/tree identity를 현재 code tree와 대조한다. 같으면 증거를 재사용한다. receipt 부재 또는 구현·hotfix로 stale이면 메인이 test/lint/build/typecheck/coverage 명령·exit/warning을 현재 revision에서 수집하고 `impl-validator:CODEBASE_SANITY`를 이번 epic의 affected scope로 재감사한다. PASS이면 메인이 현재 tree identity의 receipt를 같은 local 경로에 보존한다. local-only/ignored receipt는 code PR에 노출하지 않는다. 이 receipt는 canonical Root refresh 완료 증거나 다음 단계의 affected capability/entrypoint 현재 코드 대조를 대신하지 않는다.
