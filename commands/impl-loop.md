...
**진입 시 (MUST — §강제 사전 정합)** — TaskCreate 를 이 순서로: `task1 헤더` → `task1 sub × 2 (Hybrid A) 또는 × 5 (후보 경로)` → `task2 헤더` → … → `taskN 헤더`. task1 헤더만 in_progress. **skip 시 사용자 가시성 손실 = catastrophic 안티패턴** (§강제 사전).
