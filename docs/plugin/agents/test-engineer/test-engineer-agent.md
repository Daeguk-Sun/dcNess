...
3. `domain-model.md` 가 있으면 domain-model과 architecture에서 의존 경계를 읽는다. 없으면 epic architecture 의 모듈 목록, `docs/decisions/`, 대상 impl 문서의 module/decision references로 의존 경계를 판단하고 파일 부재만으로 `SPEC_GAP_FOUND` 하지 않는다.
