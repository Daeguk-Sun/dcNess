---
name: spec
description: 새 기능 / PRD 변경 / 큰 기획을 받아 메인 Claude 가 사용자와 직접 그릴미 대화하며 `docs/prd.md` 초안 작성 → 사용자 초안 확인 → 기술 검토 필요 영역에 항목이 있으면 `/tech-review` preflight 실행 → tech-review 결과를 반영해 PRD 최종화 → epic 단위 `docs/milestones/vNN/epics/epic-NN-<slug>/stories.md` 작성 → 사용자 최종 OK → `product-acceptance:SPEC_ACCEPTANCE` → PR 머지 → 이슈 등록 여부 확인 → `/design` 권고 시퀀스로 진행하는 `/spec` 절차. 구현 진입은 `/impl` 이 lane 을 판정하고, story/epic 구현 완료 후 `/acceptance` 로 제품 검수한다.
---

# Spec Skill — PRD / Story 작성 절차

본 스킬은 사용자-facing 기본 surface 의 `/spec` 진입점이다. 기본 surface 는 `/spec -> /design -> /impl -> /acceptance` 다.
...
라우팅 SSOT 는 [`spec-routing.md`](spec-routing.md) 다. 본 파일은 Step 운전 절차만 담는다.
