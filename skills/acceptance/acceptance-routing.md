...
flowchart TB
  ...
  TAX --> NEXT1[/impl · /architect-loop · /spec · /ux · performance · security 후속 후보]
...
`/design` alias 는 #645 surface flip 이후의 이름이다. #649 시점에는 실제 entrypoint 인 `/architect-loop` 로 라우팅하고, 사용자가 `/design` 을 따라 치게 만들지 않는다.
