...
- **🔧 /impl-loop run 격리 복원 + 사람 친화 진행 뷰 (#443, #1019 이전 기록)**: #438 in-session 복귀 시 `commands/impl-loop.md` 절차가 `loop-procedure.md` 옛 `b<i>.` 한-run 모델 잔재를 베껴 `begin-run impl-loop` 1회 + 모든 task 를 한 run 에 묶음 → `run_review` 가 run 단위라 거대 review.md 1개가 되는 문제를 고쳤다. 당시에는 task별 run 격리를 복원했지만, 현행 방향은 #1019 로 재정의되어 `/impl-loop` chain 이 task commit 을 누적하고 story PR 경계에서 code-validator/pr-reviewer 를 1회 수행한다. 진행 뷰의 평탄 리스트/재생성 규칙은 유지하되 story-runner state 가 resume 기준을 맡는다.
...
- **🗑️ /impl-loop·/impl 헤드리스 폐기 — in-session 오케스트레이터 복귀 (#438, #1019 에 의해 superseded)**: 당시 Anthropic `claude -p` / Agent SDK 과금 분리 리스크 때문에 헤드리스 경로를 폐기한다고 기록했지만, 2026-07-09 현재 결정은 다르다. #1019 기준 현행은 **일반 `/impl` = 메인 직접 구현 + 격리 `pr-reviewer`**, **`/impl-loop` = provider-agnostic headless story/epic runner** 이다. `/impl-loop` 은 task 를 commit 으로 누적하고 story 단위 PR 에서 code-validator/pr-reviewer/acceptance 를 수행한다. 따라서 이 항목의 폐기 결정과 옛 PR framing 은 과거 결정 기록일 뿐 현행 규칙이 아니다.
