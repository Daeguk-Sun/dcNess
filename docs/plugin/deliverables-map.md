...
`docs/architecture.md` 는 변경 목적/외부 trigger에서 실제 runtime entrypoint, stable capability owner, 관련 epic/decision으로 이동하는 bounded Cartography anchor다. `landed/stub/planned/deferred` 상태는 실제 코드 entrypoint 또는 accepted epic/decision 포인터로 검증 가능해야 한다. greenfield thin bootstrap 때 큰 모듈 경계와 의존 그래프를 얇게 담을 수 있지만, Story→모듈 매핑, 구현 순서, 상세 계약 본문, 전역 generated summary, Contract Ledger, ux/story 요약을 복제하지 않는다. 전역 모듈/의존/결정의 긴 설명은 `docs/decisions/` 또는 module docs 로 보낸다.
