...
- story 는 epic 단위로 로컬 설계 산출물(`docs/epics/epic-NN-<slug>/architecture.md` + `impl/NN-*.md`) 존재를 확인해 다음 액션을 구분한다. 설계 미완 epic 은 story 를 impl 후보로 내밀지 않고 `/design <epic-path>` 를 제시하고, 설계 완료 epic 만 story 를 impl 후보로 승격한다. UI epic 에서 `ux-flow.md` 는 있으나 full design pack 이 없으면 "`ux 완료 · system 미완`" 라벨로 같은 `/design <epic-path>` 를 제시한다. pack 은 완료됐지만 `revision-pending.md` 가 남아 있으면 "`UX 개정 머지됨 · system 전파 대기`" 라벨로 `/design <epic-path>` 를 제시한다 — 개정에서는 pack 이 이미 완성돼 있어 파일 존재만으로는 전파 대기를 알 수 없다.
- 전파 대기 epic 은 story 후보 경로와 **독립된 `## 전파 대기` 섹션**으로도 드러낸다. 그 epic 의 열린 story 가 없거나 전부 `in-progress` 면 story 그룹에 나타나지 않아 방치 신호가 사라지기 때문이다. 대기 중인 epic 이 없으면 이 섹션을 출력하지 않는다. 로컬에 해당 epic 산출물이 없으면(repo 밖 실행 / stale checkout) `/design` 을 단정하지 않고 판정을 보류한다.
