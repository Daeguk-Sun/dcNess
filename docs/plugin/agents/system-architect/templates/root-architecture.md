...
| 변경 목적 / 외부 trigger | 시작 entrypoint (실제 repo-relative 경로) | flow/capability owner | 다음 경계 | 상태 | 상태 증거 | 관련 epic | global decision |
|---|---|---|---|---|---|---|---|
|  |  |  |  | planned | accepted epic/decision |  |  |
...
- epic 증가 때문에 route가 커지면 root에 epic 목록을 늘리지 않고 소유 epic/module 표는 온디맨드 보조 뷰로 내린다.
