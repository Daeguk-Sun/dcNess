...
flowchart TB
  ...
  FINISH -->|preflight가 확실한 미충족을 보고한 journey| ENVUSER[수렴 직전 사용자 처분 1회]
  ENVUSER -->|환경 준비| FINISH
  ENVUSER -->|journey 검수 분리| DEFER[journey_deferred 기록]
...
| 진입점 | 재개 수단 |
|---|---|
| `--chain-state` (`/impl-loop`) | `dcness-helper end-run` 으로 현재 run 을 닫고 같은 `--chain-state` 로 chain 을 다시 호출한다. chain 이 새 run 과 `step_started` 를 만든다. `--rework` 는 이 진입점에서 사용할 수 없다. |
| `--direct-run` (`/impl`) | `--rework --resume-provider <actual-provider>` 로 같은 run 에 새 build-worker step 을 연다. |

재개 전에는 사용자가 정한 처분(환경 준비 완료, journey 검수 분리 등)을 다음 worker prompt 에 명시해 같은 지점에서 다시 멈추지 않게 한다.
