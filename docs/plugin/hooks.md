...
| 외부 mutation denylist | sub-agent 의 `git push` / `gh pr·issue (create\|merge\|edit\|close\|comment)` / `gh api -X POST\|PUT\|PATCH\|DELETE` / GitHub MCP mutation (`create_*`/`update_*`/`merge_*`/`push_*`/`add_*` 등) 차단. read-only(`gh ... view\|list`, `gh api` GET, MCP `get_*`/`list_*`/`search_*`)·메인은 통과 (#597 커밋5) |
