...
- **task_list** (Step 1): `/design` dispatcher 가 durable 산출물과 명시 개정 의도로 내부 stage 를 선택한다. stage 1 `design-ux` = 목업 선행 여부 checkpoint → ux-architect:UX_FLOW → 목업=예 한정 디자인 시스템 체크포인트 → 조건부 canvas-design → 사용자 PICK → stage 1 PR. UX 층 revision mode 에서는 기존 UX 산출물을 수술적으로 개정하고 stage 1 revision PR 뒤 `design-system` revision mode 로 전파한다. 목업 없음 / opt-out / yolo 기본값 = 목업 없음 경로는 기존 UX stage 흐름을 유지하고 canvas-design 을 강제하지 않는다. stage 2 `design-system` = Cartography freshness preflight → [기술 스택 그릴미 또는 기록된 스택 결정 확인-후-skip — 메인 직접, helper 비대상] → module-architect(epic-batch 또는 revision mode) → architecture-validator(final epic 검증). (UI-less epic) ux-architect 제외. 모듈 topology 부재 greenfield 첫 설계면 system-architect(thin bootstrap)를 module-architect 앞에 1회 추가한다. 기존 모듈 경계·불변조건·storage policy·shared public boundary·global decision 변경 신호가 명백하면 system-architect opt-in checkpoint 를 module-architect 앞에 배치하며, preflight에서 놓친 영향은 기존 `SYSTEM_CHECKPOINT_REQUIRED` fallback으로 회수한다.
...
`design-system` 진입에서는 stories, Root Cartography, 관련 global decision에서 이번 epic의 affected capability/entrypoint를 식별하고, Root의 `landed/stub/planned/deferred` 표기를 현재 코드의 runtime entrypoint와 wiring 증거에 대조한다. 이전 epic의 상태를 그대로 신뢰하지 않으며, 전체 코드를 스캔하는 새 hard gate가 아니라 이번 epic이 영향을 주는 좌표만 lazy-read 한다.
