...
`--chain-state`를 넘긴 `/impl-loop`에서만 provider capability cache가 활성화된다. story-runner state의 `chain_id`가 수명 진본이며 cache sidecar는 같은 state 디렉터리의 `provider-failure-cache/<chain_id>.json`이다. 새 chain/reinit은 새 `chain_id`를 받고, 완료 chain은 cache를 소비하지 않으며 archive/force init 시 기존 sidecar를 무효화한다. state의 `project_root`가 현재 project/worktree와 다르면 실행을 거부한다. `--chain-state`가 없는 single `/impl`과 다른 project/worktree는 cache를 공유하지 않는다. sidecar update는 file lock과 atomic replace를 사용해 같은 chain의 동시 접근에도 JSON을 손상시키지 않는다.
